...
class LINEWebhook(BaseHTTPRequestHandler):
    ...
    def reply_to_line(self, text):
        payload = json.dumps(
            {
                "replyToken": self.LINE_CHANNEL_TOKEN,
                "messages": [
                    {
                        "type": "text",
                        "text": text,
    ...
    def do_POST(self):
        ...
        try:
            ...
            if "replyToken" in event_data:
                reply_text = self.generate_message(event_data["message"]["text"])
                self.reply_to_line(event_data["replyToken"], reply_text)
